...
export function PieChartSvg({ data, colors, size = 130 }: PieChartSvgProps) {
  const [tooltip, setTooltip] = useState<{ visible: boolean; clientX: number; clientY: number; data: PieDataItem | null }>({
    visible: false,
    clientX: 0,
    clientY: 0,
    data: null,
  ...
  const onSegmentEnter = (e: React.MouseEvent, item: PieDataItem) =>
    setTooltip({
      visible: true,
      clientX: e.clientX,
      clientY: e.clientY,
  ...
  const onSegmentMove = (e: React.MouseEvent) =>
    setTooltip((t) =>
      t.visible
        ? {
            ...t,
  ...
  // Get colors with dark mode support
  const getSegmentColor = (index: number) => {
    if (index === 0) {
      // Direct - purple in dark mode, black in light mode
      return darkMode ? '#C6C7F8' : '#000000'
    }
    ...
    <>
      <div className="flex justify-center w-full">
        <div style={{ width: chartSize, height: chartSize, position: 'relative' }}>
          ...
          >
            {segmentPaths.map((pathProps, index) => (
              <path
                key={index}
                {...pathProps}
